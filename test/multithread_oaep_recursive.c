...
double test(
    ...
    for (i=0; i < times; ++i) {
        ...
        t_mixencrypt_oaep_recursive(threads, in, out, size, key, iv);

        D printf("DECRYPTION (size %lu)\n", size);
        D t_mixdecrypt_oaep_recursive(threads, out, dec, size, key, iv);
...
int main(int argc, char *argv[]) {
    ...
    for (t=1; t<=threads; t*=2) {
        printf("AESMIX-ing with OAEP RECURSIVE %s (%luB) with %d threads %d times\n",
